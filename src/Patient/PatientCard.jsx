...
const PatientCard = ({ patient }) => {
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-4">
      ...
      <p><strong>Appointment:</strong> {patient.appointmentDate}</p>
          ...
          <div className="mt-2">
            ...
            {patient.medicines.map((medicine, index) => (
              <div key={index} className="mt-2">
                <p><strong>{medicine.medicineName}</strong> ({medicine.necessityDose} {medicine.measurementUnit})</p>
                <p><strong>Schedule:</strong></p>
                <ul>
                  {medicine.schedule.map((dose, i) => (
                    <li key={i}>{dose.day} - {dose.date} - {dose.time} {dose.taken ? '✔️' : '❌'}</li>
                  ))}
                </ul>
              </div>
            ))}
